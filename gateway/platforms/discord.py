...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Set up intents.
            # Set up intents.
            # Message Content is required for normal text replies in servers.
            ...
            # bot from coming online at all, so avoid requesting them
            # unless explicitly enabled.
            #
            # Privileged intents (message_content, members, presence) must be
            # enabled in the Discord Developer Portal (Bot > Privileged Gateway Intents).
            #
            # Configure via environment variables in ~/.hermes/.env:
            #   DISCORD_INTENT_MESSAGE_CONTENT=true   — request message_content intent
            #   DISCORD_INTENT_MEMBERS=true           — request members intent
            ...
            # message_content: needed to read message text in servers (DMs always work)
            _msg_content = (
                os.getenv("DISCORD_INTENT_MESSAGE_CONTENT", "").strip().lower()
            )
            if _msg_content in ("true", "1", "yes", "on") or (
                _privileged_enabled and _msg_content != "false"
            ):
                intents.message_content = True
                logger.info(
                    "[%s] Requesting message_content privileged intent (must be enabled in Discord Developer Portal)",
                    self.name,
                )

            # members: needed for username allowlists and role lookups
            _members = os.getenv("DISCORD_INTENT_MEMBERS", "").strip().lower()
            _needs_members = any(
                not entry.isdigit() for entry in self._allowed_user_ids
            ) or bool(self._allowed_role_ids)

            if _needs_members and _members not in ("false", "0", "no", "off"):
                intents.members = True
                logger.info(
                    "[%s] Requesting members privileged intent for allowlist (must be enabled in Discord Developer Portal)",
                    self.name,
                )
            elif _members in ("true", "1", "yes", "on") or (
                _privileged_enabled and _members != "false"
            ):
                intents.members = True
                logger.info(
                    "[%s] Requesting members privileged intent (must be enabled in Discord Developer Portal)",
                    self.name,
                )
